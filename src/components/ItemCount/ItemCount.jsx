...
export default function ItemCount({stock,precio}) {
    ...
    let desabilitarBotonSuma = (cantidad === stock || stock === 0)


    let precioTotal = (cantidad * precio).toFixed(2)
 
    return (
    
        <div className='container d-flex flex-column align-items-center'> 
            <p className={`fw-bold ${stock ? "text-success" : "text-danger"}`}>Stock {`${stock ? "Disponible" : "No disponible"}`}</p>
            ...
            <Button disabled={stock === 0} className={"btn btn-primary"}>Agregar al carrito</Button>
        </div>
    )
